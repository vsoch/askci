...
The `ldap_auth` plugin allows users to login to AskCI using account information stored in an
...
To enable LDAP authentication you must:
  ...
  * Add `ldap_auth` to the `PLUGINS_ENABLED` list in `askci/settings/config.py`
  * Configure the details of your LDAP directory in `askci/settings/secrets.py`. See
    `askci/settings/dummy_secrets.py` for an example OpenLDAP configuration. A good start is to do the following:
...
To test AskCI LDAP authentication we can use a Dockerized OpenLDAP server.
...
To configure AskCI to authenticate against our LDAP directory we need to set
the following options in `askci/settings/secrets.py`:
...
Also ensure 'ldap_auth' is listed in `PLUGINS_ENABLED` inside `askci/settings/config.py`.
...
Once you have set these options, start up AskCI and you should be able to see the ldap option on the login page:
